...
> {
  ...
  render() {
    let domains = this.state.domains.sort((a, b) => {
    ...
    for (let domain of domains) {
      rows.push(
        <li
          className="p-4 flex justify-between border-b-slate-400 hover:bg-slate-200 rounded-md"
          onClick={() => {
    ...
    return (
      <ul className="grid grid-flow-row gap-8 p-4 m-4 bg-white text-slate-500 rounded-lg">
        {rows}
      </ul>
    );
  }
